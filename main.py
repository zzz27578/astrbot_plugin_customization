...
class WelcomeCustomizationPlugin(Star):
    ...
    async def _prepare_record_segments_for_send(
        ...
        for segment in self._normalize_raw_segments(segments):
            ...
            if forward_id:
                nested_segment = await self._forward_segment_with_nested_backup(
                    bot,
                    forward_id,
                    routing,
                    depth,
    ...
    async def _send_step(
        ...
        if step == "record":
            ...
            if nodes:
                await self._send_record_nodes(bot, user_id, nodes, routing, origin_group_id)
                await asyncio.sleep(DIRECT_RECORD_SETTLE_SECONDS)
                return
            raise RuntimeError("该聊天记录没有可发送内容；请重新回复原始聊天记录执行 /记录 添加 名称")
        if step == "image":
    ...
    async def _send_direct_record(
        ...
        if origin_group_id and record.get("nodes"):
        ...
        fallback_nodes = record.get("nodes") or []
        if fallback_nodes:
            try:
                started_at = int(time.time())
                await self._send_record_nodes(
    ...
    async def _record_command(self, event: AstrMessageEvent, args: list[str]) -> str:
        ...
        if action == "添加":
            ...
            root_source = await self._extract_direct_record_source(event)
            try:
            ...
            if root_source:
                self.store["records"][record_id]["root_message_id"] = root_source.get(
                    "source_message_id",
                    "",
                )
                self.store["records"][record_id]["root_source_group_id"] = root_source.get(
                    "source_group_id",
                    "",
                )
                self.store["records"][record_id]["root_source_self_id"] = root_source.get(
                    "source_self_id",
                    "",
                )
    ...
    async def _expand_forward_segments(
        ...
        for segment in self._normalize_raw_segments(segments):
            ...
            if forward_id:
                ...
                data = {"id": forward_id}
                if expanded:
                    data["_nested_nodes"] = expanded
                    data["_source_forward_id"] = forward_id
                content.append({"type": "forward", "data": data})
                continue
